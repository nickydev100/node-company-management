...
module.exports = (openRoutes, apiRoutes) => {
    ...
    openRoutes.route('/superLogin')
        .post(authCtrl.superLogin);
    ...
    apiRoutes.route('/toggleUserIsActiveById/:u_id').all(policy.isManager)
        .get(user.toggleUserIsActiveById)

    apiRoutes.route('/checkMobileAndUpdate/:u_id')
        .post(user.checkMobileAndUpdate);
    //------------------------------ APIS----------------------------//

    apiRoutes.route('/updateProfilePic').all(avatarParser.single('file'))
        .post(user.updateProfilePic)

    apiRoutes.route('/updateMe').all(policy.isManager)
        .post(user.updateMe)

    apiRoutes.route('/updateMyPassword').all(policy.isManager)
     ...
     * */
    apiRoutes.route('/listAllRoofers').all(policy.isManager)
     ...
     *          ["id1", "id2"......,"idn"]
     */
    apiRoutes.route('/listGivenRoofers')
        .post(user.listGivenRoofers)

    /**
     * List all Roofers with params as chunk and page
     * Eg. /listAllManagers?chunk=10&page=2 
     * */
    apiRoutes.route('/listAllManagers').all(policy.isManager)
     ...
     * Eg. /createRoofer 
     * */
    apiRoutes.route('/createRoofer').all(policy.isManager)
    ...
    apiRoutes.route('/createManager').all(policy.isAdmin)
    ...
    apiRoutes.route('/resetAdminPassword').all(policy.isSuperAdmin)
